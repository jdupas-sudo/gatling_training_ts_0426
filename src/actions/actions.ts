...
export const createAddToCartBodyForLoop = exec((s) => {
  try {
    const productList = JSON.parse(s.get("Products") as string);
    const picked = productList[Math.floor(Math.random() * productList.length)];

    let cartItems = s.get("CartItems")
      ? JSON.parse(s.get("CartItems") as string)
      : [];

    cartItems.push(picked);

    console.log("cart items:", cartItems);

    return s.set("CartItems", JSON.stringify(cartItems));
  } catch (error) {
    console.error("createAddToCartBody failed:", error);
    return s;
  }
